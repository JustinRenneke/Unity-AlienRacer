...
namespace ProgressBar
{
	...
	{
		private Image image;
		private Color transCyan, transMagenta;
		public float thrustBuildTransparency;
		public float thrustMaxTransparency;
		/// <summary>
		...
		void Start()
		{
			...
			transCyan = new Color(0f, 1f, 1f, thrustBuildTransparency);
			transMagenta = new Color(1f, 0f, 1f, thrustMaxTransparency);
			image.material.color = transCyan;
		}
		...
		public void SetFillerSizeAsPercentage(float Percent)
		{
			...
			if (TriggerOnComplete && isPaused && isDone)
			{
				image.material.color = transMagenta;
			}
			else
			{
				if (gameObject != null && transCyan != null && image != null && image.material != null && image.material.color != null)
					try
					{
						image.material.color = transCyan;
					}catch(Exception e)
